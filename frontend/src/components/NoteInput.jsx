...
const NoteInput = ({ onAnalysisStart, onAnalysisComplete, loading }) => {
  ...
  const handleFileAnalysis = async () => {
    ...
    try {
      ...
      if (error.response?.data?.error?.includes('OCR service temporarily unavailable')) {
        toast.error('🔄 OCR service is busy. Please try again in a moment!');
      } else if (error.response?.data?.error?.includes('Unsupported file type')) {
        toast.error('Unsupported file format. Please check the supported formats.');
      } else {
  ...
  const inputMethods = [
    ...
    { id: 'image', label: 'FREE Image OCR', icon: Image },
  ];
        ...
        <div className="space-y-4">
          <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-8 text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-4">
              {inputMethod === 'image' 
                ? '📸 Upload an image for FREE OCR text extraction!'
                : 'Upload a PDF or TXT file for analysis'
              }
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-500 mb-4">
              {inputMethod === 'image'
                ? '✨ 100% FREE OCR - No API keys required! Supports: PNG, JPG, JPEG, GIF, BMP, WEBP, TIFF (Max size: 10MB)'
